...
function makeBestMove(color,numID) {
  if (color === 'w') {
    var move = getBestMove(games[numID], color, globalSum)[0]
  } else {
    var move = getBestMove(games[numID], color, -globalSum)[0]
  }

  globalSum = evaluateBoard(games[numID], move, globalSum, 'b')
  games[numID].move(move)
  boards[numID].position(games[numID].fen())
